...
impl<T:BitVec> LPCTrie<T>{
    ...
    fn insert_impl(&mut self, key: T, prefix:u8, value:String, trie:& mut TrieNode<T>, pos:u32) ->TrieNode<T>{
        return match trie {
            TrieNode::NODE(inode) if inode.key.sub_equal(pos, inode.pos - pos, &key) => {
                ...
                let mut n = self.insert_impl(key, prefix, value,&mut inode.get_mut_child(bitpat.safe_to_usize()).lightweight_clone(),
                                             insert_pos
                );
